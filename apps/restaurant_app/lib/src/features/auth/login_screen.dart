...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  Future<void> _checkAuth() async {
    ...
    if (token != null && token.isNotEmpty) {
      ...
      final restaurantId = await _authRepository.getRestaurantId();
      
      // Verificar se role é RESTAURANT
  ...
  Future<void> login(String email, String password) async {
    ...
    try {
      ...
      if (response.user.role != 'RESTAURANT') {
        throw AuthException('Acesso negado: esta conta não é de restaurante');
      }

      // Salvar tokens
      await _authRepository.saveTokens(response.tokens.toJson());
      
      // Obter dados completos do usuário (incluindo restaurantId)
      ...
      state = state.copyWith(
        ...
        restaurantId: meData['restaurantId'],
      );
    } catch (e) {
      state = state.copyWith(
        isLoading: false,
