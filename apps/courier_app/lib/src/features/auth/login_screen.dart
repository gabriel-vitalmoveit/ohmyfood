...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  Future<void> _checkAuth() async {
    ...
    if (token != null && token.isNotEmpty) {
      ...
      final courierId = await _authRepository.getCourierId();
      
      // Verificar se role é COURIER
      if (userRole == 'COURIER') {
  ...
  Future<void> login(String email, String password) async {
    ...
    try {
      ...
      if (response.user.role != 'COURIER') {
        throw AuthException('Acesso negado: esta conta não é de estafeta');
      }

      // Salvar tokens
      await _authRepository.saveTokens(response.tokens.toJson());
      
      // Obter dados completos do usuário (incluindo courierId)
